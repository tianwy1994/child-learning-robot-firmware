...
bool MicManager::begin() {
    ...
    if (_buffer == nullptr) {
        _buffer = (uint8_t*)malloc(64 * 1024);  // 64KB fallback
        if (_buffer == nullptr) {
...
void MicManager::initI2S() {
    ...
    if (err != ESP_OK) {
        Serial.printf("[Mic] I2S 驱动安装失败: %d\n", err);
        return;
    }

    err = i2s_set_pin(I2S_NUM_1, &pin_config);
    if (err != ESP_OK) {
        Serial.printf("[Mic] I2S 引脚设置失败: %d\n", err);
...
void MicManager::stop() {
    if (_recording) {
        stopRecording(_writeOffset);  // discard return
    }
    deinitI2S();
}
...
bool MicManager::startRecording() {
    if (!_buffer) {
        Serial.println("[Mic] 错误：缓冲区未分配");
        return false;
    }

    if (_recording) {
        Serial.println("[Mic] 已在录音中，先停止当前录音");
        size_t dummy;
        stopRecording(dummy);
    }
...
bool MicManager::isRecording() {
    return _recording;
}

size_t MicManager::getRecordedSize() {
    return _writeOffset;
}

const uint8_t* MicManager::getRecordedData() {
    return _buffer;
}

bool MicManager::update() {
    if (!_recording || !_initialized || !_buffer) {
        return false;
    }
    ...
    if (err == ESP_OK && bytesRead > 0) {
        _writeOffset += bytesRead;
    }

    return true;
}

bool MicManager::isBufferFull() {
    return _writeOffset >= _bufferSize;
}
